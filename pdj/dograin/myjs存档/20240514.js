...
//普通 功能区
var fankui = false; //是否开启反馈，就是排上了要不要发弹幕
var guanli_fankui = false; //是否开启反馈，就是管理员操作了要不要发弹幕
...
function jsontoprint(data) {  
    ...
    switch (indexContent) {
        case 'danmu': //case 'DANMU_MSG':
            ...
            var ban_amminIdex =ban_admins.indexOf(person);//查找用户是不是排队黑名单
            var amminIdex = admins.indexOf(person);//查找用户是不是管理员
            var jianZ_inIdex = jianzhang.indexOf(person);//查找用户是不是舰长
            ...
            // 找到管理员,amminIdex就大于0,找不到就-1,如果我禁止排队且不是管理员,就return。
            if(all_suoyourenbukepaidui == true && amminIdex == -1){
            ...
            if(amminIdex == -1){        
                if(user_kind_room == 2){
                    ...
                    admins.push(person);
                    amminIdex = admins.indexOf(person);//更新用户管理员
                };                  
                if(user_kind_room == 1){                    
            ...
            if(amminIdex != -1){
                ...
                //拉黑 xxx
                var laheiIdex = message.indexOf("拉黑 ");//0是第一个字，1是第二个字
                console.log(laheiIdex)
                if(laheiIdex >= 0 && laheiIdex <= 1){
                ...
                //取消拉黑 xxx
                var del_laheiIdex = message.indexOf("取消拉黑 ");
                if(del_laheiIdex >= 0 && del_laheiIdex <= 1){
                ...
                if(Set_beichadui_Index >= 0){
                    var Set_beichadui_Q=message.replace(/[^0-9]/ig,"");//除了数字 全部删除
                    //条件 长度大于0，删除数字以外后剩下非空值，序号的长度大于等于需要删除的位置。
                    var Set_beichadui_MSG = message.replace(/[\d\s]+/g,"");//删除数字和空格
                    if(Set_beichadui_MSG =="设置被插队次数"){
                        jianzhang_cd_cishu = Set_beichadui_Q
            ...
            console.log("[操作之人]",person);
            console.log("[排队列表]",persons);
            console.log("[弹幕信息]",message);
            console.log("-------------------------------------",);
