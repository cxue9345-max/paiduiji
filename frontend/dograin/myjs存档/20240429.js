//版本：正式版；版本号：2024-4-29 22:58:12
//测试内容：主播默认成为插件控制者；房管可选择成为插件控制者，详细请查看更新资料。
//-------
//设置内容
//------------------------------------------------------------------------------------------
var admins =["迷糊的迷糊菇","一纸3轻予梦3","写一下你的名字哦"];//管理员
...
//普通 功能区
var fankui = false; //是否开启反馈，就是排上了要不要发弹幕(不建议开，很吵)
var guanli_fankui = false; //是否开启反馈，就是管理员操作了要不要发弹幕(不建议开，很吵)
var fangguan_can_doing = false //房管默认为插件控制者(主播已默认)
var jianzhangchadui = false; //是否开启舰长插队
var jianzhang_cd_kind = 1; //舰长插队类型|1:默认(待实装);2:上舰插队(未实装);3:礼物插队(未实装);|
...
var WX_webhook = ""; //企业微信
//未实装功能,画饼区
var paizi_paidui = false; //仅本直播间牌子可排队,需要设置,且该功能未实装。
var other_paizi_bukepaidui = false;//其他直播间牌子可排队,需要设置,且该功能未实装。
...
//-----------------------------------------------------------------------------
//测试区
//const variable = require('./peizhi.js').WX_webhook;
...
console.log("管理员:",admins);
console.log("黑名单",ban_admins);
...
function sendMessage_diy(xz_orz,kind = ""){
    console.log("排队人员：",xz_orz);
    var massage_xzorz = ",排上啦~"
    if (kind != ""){
        massage_xzorz = kind
    }
    if (fankui == true){
...
function jsontoprint(data) {
    ...
    switch (indexContent) {
            ...
            var message =data.result.msg; //言语
            var person = data.result.uname; //名称
            var is_jianzhang = data.result.svip; //是否舰长?存疑
            var user_kind_room = data.result.manager; // 0用户,1房管,2主播

            var index =persons.findIndex((item,index) => { return item.indexOf(person) != -1 });

            var ban_amminIdex =ban_admins.indexOf(person);
            if(ban_amminIdex!=-1){
                console.log("黑名单成员:",person);
                //console.log(person);
                return
            }
            ...
            if(user_kind_room == 2){   
                admins.push(person)};
                console.log("检测到主播:",person,admins)
            if(user_kind_room == 1){
                console.log("检测到管理员:",person)
                if(fangguan_can_doing == true)
                    admins.push(person)};
            if(is_jianzhang == 1){
                console.log("检测到舰长:",person)
                jianzhang.push(person)};
                
            var end_jz_Idex =persons.findLastIndex((item,index) => { return item.indexOf("舰|") != -1 });
            //var end_jz_Idex =persons.lastIndexOf("舰|");
            if(message == "排队" && is_jianzhang == 11||message == "排队" && person == "一纸轻予梦"){
                
                if(end_jz_Idex = -1){chadui_Index_is = 0};                
                if(end_jz_Idex != -1){chadui_Index_is = end_jz_Idex + 1};

                console.log("jzcd",end_jz_Idex)
                console.log("jzcd",chadui_Index_is)

                if(chadui_Index_is > -1 ){
                    chaduiText = "<span>舰|" + person +  "<br></span>" 
                    if (persons == []){
                        persons.push(person)
                    }; 
                    if (persons != []){
                        persons.splice(chadui_Index_is, 0, chaduiText);
                    };                  
                    
                    
                console.log("jzcd",person)
                console.log("jzcd",persons)               
                }
                message = "已处理完无需调用其他"
            }
